package JIRA_PostWorkLogsOnIssue
...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_PostWorkLogsOnIssue Activity.", logger.Debug, InSessionID)

	if FlowData["JiraHost"] != nil && FlowData["JiraAuthToken"] != nil && FlowData["JiraIssueID"] != nil && FlowData["JiraWorkLogComment"] != nil && FlowData["JiraWorkLogTimeInSeconds"] != nil {
		...
		if jiraAuthToken != "Unauthorized" {
			...
			if FlowData["JiraWorkLogStartTime"] != nil {
				httpServiceInput["Body"] = "{\"comment\":\"" + FlowData["JiraWorkLogComment"].(string) + "\",\"started\":\"" + FlowData["JiraWorkLogStartTime"].(string) + "\",\"timeSpentSeconds\": " + FlowData["JiraWorkLogTimeInSeconds"].(string) + "}"
			} else {
				httpServiceInput["Body"] = "{\"comment\":\"" + FlowData["JiraWorkLogComment"].(string) + "\",\"timeSpentSeconds\": " + FlowData["JiraWorkLogTimeInSeconds"].(string) + "}"
			}
	...
	if err == nil {
		msg := "Successfully posted a worklog on issue!"
	...
	logger.Log_ACT("Finished Executing JIRA_PostWorkLogsOnIssue Activity. Returning to Work Flow.", logger.Debug, InSessionID)
