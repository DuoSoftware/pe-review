package JIRA_GetIssueTransitions
...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_GetIssueTransitions Activity.", logger.Debug, InSessionID)
	...
	if err == nil {
		...
		if !httpActivityResult.ActivityStatus {
			...
			s := reflect.ValueOf(data["transitions"])
			...
			for i := 0; i < s.Len(); i++ {
				...
				delete(newMap, "to")
				delete(newMap, "hasScreen")
			}

			FlowData["JiraIssueTransitionOptions"] = interfaceList
		}
	}

	if err == nil {
		msg := "Successfully Retrieved Issue Transition Options!"
		activityContext.ActivityStatus = true
		activityContext.Message = msg
		activityContext.ErrorState = activityError
		logger.Log_ACT(msg, logger.Debug, logger.Debug, InSessionID)
	} else {
		msg := "Error : " + err.Error()
	...
	logger.Log_ACT("Finished Executing JIRA_GetIssueTransitions Activity. Returning to Work Flow.", InSessionID)
