...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_IssueTransition Activity.", logger.Debug, InSessionID)

	if FlowData["JiraHost"] != nil && FlowData["JiraAuthToken"] != nil && FlowData["JiraIssueID"] != nil && FlowData["JiraIssueTransitionOptionID"] != nil {
		...
		if jiraAuthToken != "Unauthorized" {
			...
			completeurl := jiraHostID + "/rest/api/2/issue/" + FlowData["JiraIssueID"].(string) + "/transitions"

			httpServiceInput["InSessionID"] = InSessionID
			httpServiceInput["URL"] = completeurl
			httpServiceInput["Method"] = "POST"

			httpServiceInput["Body"] = "{\"transition\":{\"id\":\"" + FlowData["JiraIssueTransitionOptionID"].(string) + "\"}}"
	...
	if err == nil {
		msg := "Successfully transitioned the Issue Status!"
	...
	logger.Log_ACT("Finished Executing JIRA_IssueTransition Activity. Returning to Work Flow.", logger.Debug, InSessionID)
