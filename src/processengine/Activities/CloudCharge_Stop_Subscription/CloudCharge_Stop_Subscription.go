package CloudCharge_Stop_Subscription
...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("CloudCharge_Stop_Subscription", logger.Debug, sessionId)

	var err error
	var host string
	httpServiceInput := make(map[string]interface{})

	if FlowData["Host"] != nil && FlowData["IsImmediateCancel"] != nil && FlowData["securityToken"] != nil {
		host = FlowData["Host"].(string)
		host = strings.TrimSpace(host)
		host = strings.TrimSuffix(host, "/")
		completeurl := host + "/duosoftware.paymentgateway.service/stripe/permanentDisconnect"

		if FlowData["IsImmediateCancel"].(bool) == true {
			inputJson["action"] = "immediate"
		} else {
			inputJson["action"] = "eod"
		}
