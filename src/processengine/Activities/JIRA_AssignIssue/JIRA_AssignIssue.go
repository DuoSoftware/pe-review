...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_AssignIssue Activity.", logger.Debug, sessionId)

	if FlowData["JiraHost"] != nil && FlowData["JiraAuthToken"] != nil && FlowData["JiraIssueID"] != nil && FlowData["JiraAssignee"] != nil {
		...
		if jiraAuthToken != "Unauthorized" {
			...
			completeurl := jiraHostID + "/rest/api/2/issue/" + FlowData["JiraIssueID"].(string) + "/assignee"
			...
			object["name"] = FlowData["JiraAssignee"].(string)
			objectBytes, _ := json.Marshal(object)

			httpServiceInput["Body"] = string(objectBytes)
	...
	if err == nil {
		msg := "Successfully Assigned person to Issue!"
	...
	logger.Log_ACT("Finished Executing JIRA_AssignIssue Activity. Returning to Work Flow.", logger.Debug, sessionId)
