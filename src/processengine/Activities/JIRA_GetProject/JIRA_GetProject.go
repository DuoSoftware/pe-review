...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_GetProject Activity.", logger.Debug, InSessionID)

	if FlowData["JiraHost"] != nil && FlowData["JiraAuthToken"] != nil && FlowData["JiraProjectID"] != nil {
		...
		if jiraAuthToken != "Unauthorized" {
			...
			completeurl := jiraHostID + "/rest/api/2/project/" + FlowData["JiraProjectID"].(string)
	...
	if err == nil {
		...
		if !httpActivityResult.ActivityStatus {
			...
			FlowData["JiraProject"] = data
		}
	}

	if err == nil {
		msg := "Successfully Retrieved Project for user!"
	...
	logger.Log_ACT("Finished Executing JIRA_GetProject Activity. Returning to Work Flow.", logger.Debug, InSessionID)
