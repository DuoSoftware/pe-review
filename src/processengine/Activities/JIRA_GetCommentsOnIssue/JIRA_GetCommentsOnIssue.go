package JIRA_GetCommentsOnIssue
...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("Executing JIRA_GetCommentsOnIssue Activity.", logger.Debug, InSessionID)
	...
	if err == nil {
		...
		if !httpActivityResult.ActivityStatus {
			...
			s := reflect.ValueOf(data["comments"])
			...
			FlowData["JiraCommentsOnIssue"] = interfaceList
		}
	}

	if err == nil {
		msg := "Successfully retrieved all comments on issue!"
	...
	logger.Log_ACT("Finished Executing JIRA_GetCommentsOnIssue Activity. Returning to Work Flow.", logger.Debug, InSessionID)
