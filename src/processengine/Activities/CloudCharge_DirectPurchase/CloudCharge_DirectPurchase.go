package CloudCharge_DirectPurchase
...
func Invoke(FlowData map[string]interface{}) (flowResult map[string]interface{}, activityResult *context.ActivityContext) {
	...
	logger.Log_ACT("CloudCharge_DirectPurchase", logger.Debug, sessionId)
	...
	if FlowData["Host"] != nil &&
		...
		FlowData["token"] != nil &&
		FlowData["amount"] != nil &&
		FlowData["order_id"] != nil &&
		FlowData["securityToken"] != nil {
		host = FlowData["Host"].(string)
		host = strings.TrimSpace(host)
		host = strings.TrimSuffix(host, "/")
		completeurl := host + "/duosoftware.paymentgateway.service/stripe/directPurchase"

		inputJson["customer"] = FlowData["customer"]
		inputJson["token"] = FlowData["token"]
		inputJson["amount"] = FlowData["amount"]
		inputJson["order_id"] = FlowData["order_id"]
