...
func (g GetByKey) Invoke(parameters map[string]interface{}) *context.ActivityContext {
	...
	//url := "http://" + Common.GetObjectstoreIP() + "/" + domain + "/" + class + "/" + key
	url := "http://" + domain + "/data/" + domain + "/" + class + "/" + key + "?securityToken=" + securityToken
	...
	if err != nil {
		...
		objArray = make(map[string]interface{})
		...
		if len(response.Result) > 0 {
			...
			activityContext.ActivityStatus = false
			activityContext.Message = "Record Not Found!"
			activityContext.ErrorState = activityError
		}
	}
